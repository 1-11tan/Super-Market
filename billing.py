...
class billing:
        ...
        if self.txt_customer_name.get()==" ":
            ...
            for row in reader:
                ...
                if str(row[0])==str(self.txt_pid.get()):
                    ...
                    self.total_price=self.total_price+int(price)*int(self.txt2_qty.get())
            ...
            if flag==True:
                ...
                self.txt_total_price.insert(0,str(self.total_price))
    ...
    def __init__(self):
        ...
        self.lb_total_payable = tkinter.Label(self.p3, text="Net Payable")
        ...
        self.lb_date=tkinter.Label(self.p1,text="Enter Date")
        ...
        self.lb_pid = tkinter.Label(self.p1, text="Enter Product id")
        self.txt_pid = tkinter.Entry(self.p1)

        self.lb_customer_pno= tkinter.Label(self.p1, text="Enter Customer Phone Number")
        self.txt_customer_pno= tkinter.Entry(self.p1)

        self.lb2_qty = tkinter.Label(self.p1, text="Quantity")
        self.txt2_qty = tkinter.Entry(self.p1)

        self.t1=tkinter.ttk.Treeview(self.p2,column=("srno","pid","pname","qty","price","discount","netprice","totalprice"))
        self.t1.column("srno", width=15)
        self.t1.heading("srno", text="Sr No.")
        ...
        self.lb_billid.grid(row=0, column=2)
        self.txt_billid.grid(row=0, column=3)

        self.lb_customer_name.grid(row=2, column=2)
        self.txt_customer_name.grid(row=2, column=3)

        self.lb_customer_pno.grid(row=2,column=4)
        self.txt_customer_pno.grid(row=2,column=5)
